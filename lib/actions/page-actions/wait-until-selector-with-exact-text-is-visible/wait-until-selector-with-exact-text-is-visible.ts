...
export async function waitUntilSelectorWithExactTextIsVisible(
  ...
  try {
    await page.waitFor(
      ({ selector, text }) => {
        ...
        const foundElements = elements.filter((element) => {
          ...
          if (htmlElement && htmlElement.innerText && htmlElement.innerText.trim() === text) {
            return true;
          }
          return false;
        });
    ...
    if (
      ...
      throw new Error(
        `Error: waiting for selector '${selector}' with exact text '${text}' failed: timeout ${options.timeoutInMilliseconds}ms exceeded`,
      );
    }
    throw error;
  }
