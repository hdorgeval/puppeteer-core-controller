...
import { defaultWaitOptions } from './wait-until-selector-is-visible';
...
describe('wait until selector is visible', (): void => {
  ...
  test('should return an error when page has not been initalized', async (): Promise<void> => {
    ...
    await SUT.waitUntilSelectorIsVisible('foobar', defaultWaitOptions, page).catch((error): void =>
  ...
  test('should find selector before timeout occurs', async (): Promise<void> => {
    ...
    // When
    // Then
    await SUT.waitUntilSelectorIsVisible('a.btn', defaultWaitOptions, page).catch((error) => {
      throw error;
    });
  });
