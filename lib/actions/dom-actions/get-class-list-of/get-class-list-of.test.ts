...
describe('get class list', (): void => {
  ...
  test('should return an error when page has not been initalized', async (): Promise<void> => {
    ...
    const expectedError = new Error(
      "Error: cannot get the class list of 'foobar' because a new page has not been created",
    );
    await SUT.getClassListOf('foobar', page).catch((error): void =>
  ...
  test('should return an error when selector is not found', async (): Promise<void> => {
    ...
    await SUT.getClassListOf('selector', page).catch((error): void =>
      expect(error).toMatchObject(expectedError),
    );
  });
});
