...
export async function querySelectorWithExactText(
  selector: string,
  text: string,
  ...
  for (let index = 0; index < elements.length; index++) {
    ...
    const innerText = await element.evaluate((node): string | undefined => {
      ...
      if (nodeWithText && nodeWithText.innerText) {
        return nodeWithText.innerText.trim();
      }
  ...
  throw new Error(`Error: Cannot find a selector '${selector}' with exact text '${text}'`);
}
